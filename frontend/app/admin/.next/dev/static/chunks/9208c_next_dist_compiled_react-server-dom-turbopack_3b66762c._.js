(globalThis.TURBOPACK || (globalThis.TURBOPACK = [])).push([
	...
	"[project]/frontend/node_modules/.pnpm/next@16.0.10_@babel+core@7.29.0_@playwright+test@1.59.1_babel-plugin-react-compiler@1.0_0b87681441210d9226c08a732ad4ea16/node_modules/next/dist/compiled/react-server-dom-turbopack/cjs/react-server-dom-turbopack-client.browser.development.js [app-client] (ecmascript)",
			...
			(function () {
				...
				function preloadModule(metadata) {
					for (
						...
						if (!instrumentedChunks.has(thenable)) {
							var resolve = loadedChunks.add.bind(
								loadedChunks,
								thenable,
							);
				...
				function describeObjectForErrorMessage(
					...
					if (isArrayImpl(objectOrArray))
						...
						if (jsxPropsParents.has(objectOrArray)) {
							...
							objKind =
								"<" + (describeElementType(objKind) || "...");
							type = Object.keys(objectOrArray);
							for (i = 0; i < type.length; i++) {
								objKind += " ";
								value = type[i];
								objKind +=
									describeKeyForErrorMessage(value) + "=";
								...
								value === expandedName
									...
									: (objKind =
											10 > _substr2.length &&
											40 >
												objKind.length + _substr2.length
												? objKind + _substr2
												: objKind + "...");
							}
							objKind += ">";
					...
					try {
						switch (typeof value) {
							...
							case "object":
								...
								if ("string" === typeof value.url)
									return value.url;
								if ("string" === typeof value.href)
									return value.href;
								if ("string" === typeof value.src)
									return value.src;
								if ("string" === typeof value.currentSrc)
									return value.currentSrc;
								if ("string" === typeof value.command)
								...
								return str.startsWith("[object ") ||
									...
									: str;
							case "string":
								return 5 > value.length || 500 < value.length
									? ""
									: value;
							case "number":
							case "bigint":
				...
				function getIOShortName(ioInfo, description, env, rootEnv) {
					...
					if (1 < rootEnv) {
						...
						if (0 < l && l <= rootEnv)
							...
							47 === description.charCodeAt(queryIdx - 1) &&
								queryIdx--;
							desc = description.lastIndexOf("/", queryIdx - 1);
							queryIdx - desc < rootEnv
								? (desc =
										" (\u2026" +
										description.slice(desc, queryIdx) +
										")")
								: ((l = description.slice(
										desc,
										desc + rootEnv / 2,
				...
				function logComponentAwait(
					...
					if (supportsUserTiming && 0 < endTime) {
						...
						var debugTask =
							asyncInfo.debugTask || asyncInfo.awaited.debugTask;
						if (debugTask) {
							var properties = [];
				...
				function filterDebugInfo(response, value) {
					if (null !== response._debugEndTime) {
						response =
							response._debugEndTime - performance.timeOrigin;
						for (
							var debugInfo = [], i = 0;
							i < value._debugInfo.length;
							i++
						) {
				...
				function moveDebugInfoFromChunkToInnerValue(chunk, value) {
					...
					"object" !== typeof value ||
						...
						isArrayImpl(value._debugInfo)
							? value._debugInfo.unshift.apply(
									value._debugInfo,
									chunk,
								)
							...
							if ("resolved_model" === debugChunk.status) {
								for (
									var idx = debugInfo.length,
										c = debugChunk._debugChunk;
									null !== c;
								)
									("fulfilled" !== c.status && idx++,
										(c = c._debugChunk));
				...
				function initializeElement(response, element, lazyNode) {
					...
					null !== lazyNode &&
						...
						"fulfilled" === lazyNode._payload.status &&
							...
							element._debugInfo
								? element._debugInfo.unshift.apply(
										element._debugInfo,
										response,
									)
									...
									switch (referencedChunk.status) {
										...
										case "pending":
											...
											null === referencedChunk.value
												? (referencedChunk.value = [
														reference,
													])
												: referencedChunk.value.push(
														reference,
													);
					...
					if (null !== parentChunk) {
						...
						for (var i = 0; i < referencedChunk.length; ++i) {
							...
							null == debugInfoEntry.name &&
								parentChunk.push(debugInfoEntry);
						}
					}
				}
					...
					if ("$" === value[0]) {
						...
						switch (value[1]) {
							...
							case "L":
								return (
									...
									null !== initializingChunk &&
										...
										initializingChunk._children.push(
											response,
										),
							...
							case "E":
								...
								try {
									if (
										!mightHaveStaticConstructor.test(
											response,
										)
									)
				...
				function ResponseInstance(
					...
					this._debugEndTime =
						null == debugEndTime ? null : debugEndTime;
					...
					debugChannel &&
						(null === debugChannelRegistry
							? (closeDebugChannel(debugChannel),
								(this._debugChannel = void 0))
							: debugChannelRegistry.register(
									this,
									debugChannel,
									this,
								));
				...
				function incrementChunkDebugInfo(streamState, chunkLength) {
					...
					endTime > previousEndTime + 10
						...
						: ((debugInfo.end = endTime),
							(debugInfo.byteSize = chunkLength));
				}
						...
						type,
						{
							...
							enqueueModel: function (json) {
								if (null === previousBlockedChunk) {
									var chunk = createResolvedModelChunk(
										response,
										json,
									);
									initializeModelChunk(chunk);
					...
					return rootTask
						? response._rootEnvironmentName !== childEnvironmentName
							? ((response = console.createTask.bind(
									console,
									'"use ' +
										childEnvironmentName.toLowerCase() +
										'"',
								)),
				...
				function initializeFakeTask(response, debugInfo) {
					if (!supportsCreateTask || null == debugInfo.stack)
						return null;
						...
						env !== cachedEntry
							...
							: void 0 !== debugInfo.key
								...
								: void 0 !== debugInfo.name
									...
									: "await " +
										(debugInfo.awaited.name || "unknown");
				...
				function initializeDebugInfo(response, debugInfo) {
					...
					if (
						null == debugInfo.owner &&
						null != response._debugRootOwner
					) {
						var _componentInfoOrAsyncInfo = debugInfo;
						_componentInfoOrAsyncInfo.owner =
							response._debugRootOwner;
						_componentInfoOrAsyncInfo.stack = null;
						_componentInfoOrAsyncInfo.debugStack =
							response._debugRootStack;
						_componentInfoOrAsyncInfo.debugTask =
						...
						if (null == blockedChunk)
							((blockedChunk = createResolvedModelChunk(
								...
								"fulfilled" === blockedChunk.status
									...
									: (blockedChunk.then(
											function (v) {
												return replayConsoleWithCallStackInDEV(
													response,
													v,
												);
											},
				...
				function flushComponentPerformance(
					...
					if (0 === debugInfo.length && "fulfilled" === root.status) {
						...
						"object" === typeof resolvedValue &&
							...
							(isArrayImpl(resolvedValue) ||
								"function" ===
									typeof resolvedValue[ASYNC_ITERATOR] ||
					...
					if (debugInfo) {
						for (
							var startTime$jscomp$0 = 0, i = 0;
							i < debugInfo.length;
							i++
						) {
							var info = debugInfo[i];
							"number" === typeof info.time &&
								(startTime$jscomp$0 = info.time);
							if ("string" === typeof info.name) {
								startTime$jscomp$0 < trackTime &&
									trackIdx$jscomp$6++;
					...
					for (
						...
						childEndTime > childTrackTime &&
							(childTrackTime = childEndTime);
						childEndTime > childrenEndTime &&
							...
							if ("number" === typeof _info2.time) {
								...
								if (-1 < endTimeIdx)
									for (var j = endTimeIdx - 1; j > _i6; j--) {
										...
										if (
											"string" ===
											typeof candidateInfo.name
										) {
											componentEndTime >
												childrenEndTime &&
												(childrenEndTime =
													componentEndTime);
											var componentInfo$jscomp$0 =
													candidateInfo,
												response = response$jscomp$0,
												componentInfo$jscomp$1 =
											...
											if (
												...
												var componentInfo$jscomp$2 =
														componentInfo$jscomp$1,
													trackIdx$jscomp$1 =
														trackIdx$jscomp$0,
													startTime$jscomp$2 =
														startTime$jscomp$1,
													childrenEndTime$jscomp$1 =
														childrenEndTime$jscomp$0,
													error = root.reason;
												if (supportsUserTiming) {
													var env =
															componentInfo$jscomp$2.env,
														name =
															componentInfo$jscomp$2.name,
														entryName$jscomp$0 =
															env ===
																response._rootEnvironmentName ||
															void 0 === env
												...
												if (
													...
													var env$jscomp$0 =
															componentInfo$jscomp$3.env,
														name$jscomp$0 =
															componentInfo$jscomp$3.name,
														isPrimaryEnv =
															env$jscomp$0 ===
															response._rootEnvironmentName,
														selfTime =
																...
																var properties$jscomp$1 =
																		[
																			[
																				"Rejected",
																				"object" ===
																					typeof error$jscomp$0 &&
																				null !==
																					error$jscomp$0 &&
																				"string" ===
																					typeof error$jscomp$0.message
																					? String(
																							error$jscomp$0.message,
																						)
																					: String(
																							error$jscomp$0,
																						),
									...
									for (
										...
										if (
											...
											var _componentInfo = _candidateInfo,
												_env =
													response$jscomp$0._rootEnvironmentName,
												componentInfo$jscomp$4 =
													_componentInfo,
												trackIdx$jscomp$4 =
													trackIdx$jscomp$6,
												startTime$jscomp$5 = time,
												childrenEndTime$jscomp$3 =
													childrenEndTime;
											if (supportsUserTiming) {
												var env$jscomp$2 =
														componentInfo$jscomp$4.env,
													name$jscomp$1 =
														componentInfo$jscomp$4.name,
													entryName$jscomp$2 =
														env$jscomp$2 === _env ||
														void 0 === env$jscomp$2
															? name$jscomp$1
							...
							"fulfilled" !== id.status &&
								...
								"blocked" !== tag.status ||
									...
									((streamState = row
										...
										.split(":")),
									(streamState = parseInt(
										streamState[0],
										16,
									)),
									"pending" ===
										getChunk(response, streamState)
											.status &&
										(id._debugChunk = null)));
							break;
						case 74:
				...
				function createResponseFromOptions(options) {
					...
					return new ResponseInstance(
						...
						options && options.environmentName
							...
							: void 0,
						options && null != options.startTime
							? options.startTime
							: void 0,
						options && null != options.endTime
							? options.endTime
							: void 0,
						debugChannel,
					)._weakResponse;
					...
					function progress(_ref) {
						...
						if (value instanceof ArrayBuffer)
							...
							if (
								...
								var response =
										unwrapWeakResponse(response$jscomp$0),
									i = 0,
	...
	"[project]/frontend/node_modules/.pnpm/next@16.0.10_@babel+core@7.29.0_@playwright+test@1.59.1_babel-plugin-react-compiler@1.0_0b87681441210d9226c08a732ad4ea16/node_modules/next/dist/compiled/react-server-dom-turbopack/client.browser.js [app-client] (ecmascript)",
	(__turbopack_context__, module, exports) => {
		"use strict";
