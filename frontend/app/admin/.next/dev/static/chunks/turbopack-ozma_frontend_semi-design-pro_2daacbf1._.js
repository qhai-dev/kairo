(globalThis.TURBOPACK || (globalThis.TURBOPACK = [])).push([
	...
	{
		otherChunks: [
			"static/chunks/[turbopack]_browser_dev_hmr-client_hmr-client_ts_3cd7519b._.js",
			"static/chunks/83328_next_dist_compiled_react-dom_7343d7c2._.js",
			"static/chunks/83328_next_dist_compiled_react-server-dom-turbopack_0b929c63._.js",
			"static/chunks/83328_next_dist_compiled_next-devtools_index_fa1b31ae.js",
			"static/chunks/83328_next_dist_compiled_e887abf4._.js",
			"static/chunks/83328_next_dist_client_dc4a563b._.js",
			"static/chunks/83328_next_dist_8a678648._.js",
			"static/chunks/558fe_@swc_helpers_cjs_f9fe9c21._.js",
		],
		runtimeModuleIds: [
...
importScripts(...self.TURBOPACK_NEXT_CHUNK_URLS.map(c => self.TURBOPACK_WORKER_LOCATION + c).reverse());`;
		...
		for (const moduleId of invalidated) {
			...
			switch (effect.type) {
				...
				default:
					invariant(
						effect,
						(effect) => `Unknown effect type: ${effect?.type}`,
					);
			}
		}
	...
	function applyUpdate(update) {
		switch (update.type) {
			...
			default:
				invariant(
					update,
					(update) => `Unknown update type: ${update.type}`,
				);
		}
	}
				...
				switch (chunkUpdate.type) {
					...
					default:
						invariant(
							chunkUpdate,
							(chunkUpdate) =>
								`Unknown chunk update type: ${chunkUpdate.type}`,
						);
				}
			}
		}
	...
	function applyEcmascriptMergedUpdate(update) {
		...
		const { disposedModules } = updateChunksPhase(
			chunksAdded,
			chunksDeleted,
		);
		...
		for (const [chunkPath, mergedChunkUpdate] of Object.entries(updates)) {
			switch (mergedChunkUpdate.type) {
				...
				case "deleted": {
					...
					const updateDeleted = new Set(
						chunkModulesMap.get(chunkPath),
					);
			...
			reloadChunk(chunkUrl) {
				return new Promise((resolve, reject) => {
					if (!isCss(chunkUrl)) {
						reject(
							new Error(
								"The DOM backend can only reload CSS chunks",
							),
						);
						return;
					}
